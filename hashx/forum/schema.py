...
class RelayQuery(graphene.ObjectType):
    question_likes = graphene.List(QuestionNode)
    
    all_likes = ViewAllAuthenticatedQuery(LikeNode, filterset_class=LikeFilter)
    likes = AuthenticatedNode.Field(LikeNode)
    all_replies = ViewAllAuthenticatedQuery(
        ReplyNode, filterset_class=ReplyFilter)
    replies = AuthenticatedNode.Field(ReplyNode)
    all_questions = ViewAllAuthenticatedQuery(
        QuestionNode, filterset_class=QuestionFilter)
    questions = AuthenticatedNode.Field(QuestionNode)
    all_categories = ViewAllAuthenticatedQuery(
